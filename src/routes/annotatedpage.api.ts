...
router.post('/newpage', async (req, res) => {
  ...
  return newPage
    .save()
    .then(() => res.status(200).json({ success: true, content: article }))
...
router.put('/update', async (req, res) => {
  ...
  return AnnotatedPage.findByIdAndUpdate(
    filter,
    update,
    {
      new: true,
...
router.get('/getmissingsummaries', async (req, res) => {
  return AnnotatedPage.find(
    { summary: { $not: { $exists: true } } },
    (err, library) => {
      if (err) return err;
